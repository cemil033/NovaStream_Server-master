...
public class SerialViewModel : ViewModelBase
{
    ...
    private async Task Delete(Serial serial)
    {
        ...
        try
        {
            var episodes = _dbContext.Episodes.Include(e => e.Season).Where(e => e.Season.SerialId == serial.Id);
            ...
            if (SeasonViewModel.isCreated)
            {
                ...
                foreach (var season in model.Seasons)
                    if (season.SerialId == serial.Id) items.Add(season);
    ...
    private async Task OpenEditDialogHost(Serial serial)
    {
        ...
        {
            var season = await _dbContext.Seasons.FirstOrDefaultAsync(s => s.SerialId == serial.Id && s.Number == 1);
