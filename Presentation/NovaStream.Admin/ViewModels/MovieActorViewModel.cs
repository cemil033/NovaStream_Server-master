...
public class MovieActorViewModel : ViewModelBase
{
    ...
    public async Task Initialize()
    {
        ...
        try
        {
            MovieActors = new ObservableCollection<MovieActor>(_dbContext.MovieActors.Include(ma => ma.Actor).Where(ma => ma.MovieId == Movie.Id));
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.MovieActors.Include(ma => ma.Actor).Where(ma => ma.MovieId == Movie.Id).ToList() :
            _dbContext.MovieActors.Include(ma => ma.Actor).Where(ma => ma.MovieId == Movie.Id && ma.Actor.Name.Contains(pattern)).ToList();
    ...
    private async Task Delete(MovieActor movieActor)
    {
        ...
        _ = MessageBoxService.Show($"Delete <{movieActor.Actor.Name} {movieActor.Actor.Surname} from {movieActor.Movie.Name}>...", MessageBoxType.Progress);
    ...
    private async Task OpenAddDialogHost()
    {
        ...
        var existsActors = await _dbContext.MovieActors.Include(ma => ma.Actor).Where(ma => ma.MovieId == Movie.Id).Select(ma => ma.Actor).ToListAsync();
