...
public class EpisodeViewModel : ViewModelBase
{
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.Episodes.Include(e => e.Season).ToArray().Where(e =>
                string.Format("{0} S{1:00} E{2:00}", e.Season.Serial.Name, e.Season.Number, e.Number).Contains(pattern) ||
                string.Format("{0} {1}", e.Season.Serial.Name, e.Season.Number, e.Name).Contains(pattern)).ToList();
    ...
    private async Task Delete(Episode episode)
    {
        ...
        _ = MessageBoxService.Show(string.Format("Delete <{0} S{1:00} E{2:00}>...", episode.Season.Serial.Name, episode.Season.Number, episode.Number), MessageBoxType.Progress);
    ...
    private async Task OpenEditDialogHost(Episode episode)
    {
        ...
        model.Episode.Serial = _dbContext.Serials.FirstOrDefault(s => s.Name == episode.Season.Serial.Name);
