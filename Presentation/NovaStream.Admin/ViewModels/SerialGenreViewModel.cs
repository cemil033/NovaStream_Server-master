...
public class SerialGenreViewModel : ViewModelBase
{
    ...
    public async Task Initialize()
    {
        ...
        try
        {
            SerialGenres = new ObservableCollection<SerialGenre>(_dbContext.SerialGenres.Include(sg => sg.Genre).Where(sg => sg.SerialId == Serial.Id));
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.SerialGenres.Include(sg => sg.Genre).Where(sg => sg.SerialId == Serial.Id).ToList() :
            _dbContext.SerialGenres.Include(sg => sg.Genre).Where(sg => sg.SerialId == Serial.Id && sg.Genre.Name.Contains(pattern)).ToList();
    ...
    private async Task Delete(SerialGenre serialGenre)
    {
        ...
        _ = MessageBoxService.Show($"Delete <{serialGenre.Genre.Name} from {serialGenre.Serial.Name}>...", MessageBoxType.Progress);
    ...
    private async Task OpenAddDialogHost()
    {
        ...
        var existsGenres = await _dbContext.SerialGenres.Include(sg => sg.Genre).Where(sg => sg.SerialId == Serial.Id).Select(sg => sg.Genre).ToListAsync();
