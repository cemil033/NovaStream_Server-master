...
public class SerialActorViewModel : ViewModelBase
{
    ...
    public async Task Initialize()
    {
        ...
        try
        {
            SerialActors = new ObservableCollection<SerialActor>(_dbContext.SerialActors.Include(sa => sa.Actor).Where(sa => sa.SerialId == Serial.Id));
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.SerialActors.Include(sa => sa.Actor).Where(sa => sa.SerialId == Serial.Id).ToList() :
            _dbContext.SerialActors.Include(sa => sa.Actor).Where(sa => sa.SerialId == Serial.Id && sa.Actor.Name.Contains(pattern)).ToList();
    ...
    private async Task Delete(SerialActor serialActor)
    {
        ...
        _ = MessageBoxService.Show($"Delete <{serialActor.Actor.Name} {serialActor.Actor.Surname} from {serialActor.Serial.Name}>...", MessageBoxType.Progress);
    ...
    private async Task OpenAddDialogHost()
    {
        ...
        var existsActors = await _dbContext.SerialActors.Include(sa => sa.Actor).Where(sa => sa.SerialId == Serial.Id).Select(sa => sa.Actor).ToListAsync();
