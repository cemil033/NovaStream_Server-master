...
public class MovieGenreViewModel : ViewModelBase
{
    ...
    public async Task Initialize()
    {
        ...
        try
        {
            MovieGenres = new ObservableCollection<MovieGenre>(_dbContext.MovieGenres.Include(mg => mg.Genre).Where(mg => mg.MovieId == Movie.Id));
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.MovieGenres.Include(mg => mg.Genre).Where(mg => mg.MovieId == Movie.Id).ToList() :
            _dbContext.MovieGenres.Include(mg => mg.Genre).Where(mg => mg.MovieId == Movie.Id && mg.Genre.Name.Contains(pattern)).ToList();
    ...
    private async Task Delete(MovieGenre movieGenre)
    {
        ...
        _ = MessageBoxService.Show($"Delete <{movieGenre.Genre.Name} from {movieGenre.Movie.Name}>...", MessageBoxType.Progress);
    ...
    private async Task OpenAddDialogHost()
    {
        ...
        var existsGenres = await _dbContext.MovieGenres.Include(mg => mg.Genre).Where(mg => mg.MovieId == Movie.Id).Select(mg => mg.Genre).ToListAsync();
