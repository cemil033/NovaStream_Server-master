...
public class SeasonViewModel : ViewModelBase
{
    ...
    private async Task Search(string pattern)
    {
        ...
        try
        {
            ...
            _dbContext.Seasons.ToArray().Where(s => string.Format("{0} S{1:00}", s.Serial.Name, s.Number).Contains(pattern)).ToList();
    ...
    private async Task Delete(Season season)
    {
        ...
        _ = MessageBoxService.Show(string.Format("Delete <{0} S{1:00}>...", season.Serial.Name, season.Number), MessageBoxType.Progress);
        ...
        try
        {
            ...
            var lastSeasonNumber = _dbContext.Seasons.Where(s => s.SerialId == season.SerialId).Max(e => e.Number);
